...
def test_lord_has_capability_returns_false_for_side_wide_card() -> None:
    ...
    s = load_scenario("scenario_a_toledo_beset")
    # C22 is a side_wide Capability; force-add to alfonso's caps list and
    # verify the helper still says False.
    s.lords["alfonso"].capabilities.append("C22")
    assert lord_has_capability(s, "alfonso", "C22") is False
...
def test_side_has_capability_with_in_play_card() -> None:
    ...
    assert side_has_capability(s, "christian", "C22") is True
    assert side_has_capability(s, "muslim", "C22") is False
...
def test_capabilities_for_lord_filters_wrong_scope() -> None:
    ...
    s.lords["alfonso"].capabilities.append("C22")  # C22 is side_wide
    # Defensive filter strips it
    assert "C22" not in capabilities_for_lord(s, "alfonso")
...
def test_capabilities_for_side() -> None:
    ...
    assert set(capabilities_for_side(s, "christian")) == {"C22", "C13"}
...
def test_any_capability_dispatches_by_scope() -> None:
    ...
    s.decks.capabilities_in_play.append(CardInPlay(
        card_id="C22", scope="side_wide", owner_side="christian"))
    assert any_capability(s, "christian", "C22") is True
    assert any_capability(s, "christian", "C22", lord_id="alfonso") is True
    assert any_capability(s, "muslim", "C22") is False
